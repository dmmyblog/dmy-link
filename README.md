...
### v1.3.5
✅AES加密模式: 使用AES-256-CBC加密，永久有效
✅新增样式风格：jump风格、tiktok风格、moxing风格
✅动态内容支持: JavaScript实时转换动态加载的链接
✅输入验证: 所有用户输入经过WordPress安全函数处理
✅URL验证: 使用`esc_url()`确保输出安全
✅XSS防护: 模板输出使用安全转义
✅访问控制: 开关控制和权限管理
✅ WordPress transient存储机制
✅适配子比主题以及7b2主题的圈子
💻更新了共同开发 天无神话
...
✅ 修复相对路径链接识别异常，/path/page 等路径现可正确归类为内部链接
✅ 修复与部分主题或插件的兼容性冲突
✅ 更新 Referer 防护机制，强化来源校验，降低绕过风险
✅ 提升整体兼容性与运行稳定性
✅ 优化后台菜单位置，设置入口更易找到
💻 更新来自天无神话
...
🟠 **安全修复**：
✅ AJAX 接口 `dmylink_ajax_convert` 增加 Referer 校验防止外部滥用（兼容 Nginx 缓存环境，不使用 PHP 输出的 nonce）
