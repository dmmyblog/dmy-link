...
- **角标文字**可自定义（默认「广告」，可留空）；可选**仅对未登录访客展示**
...
🟣 **新功能：跳转页广告位**：<br>
✅ 新增「跳转页广告」设置面板，支持「图片 + 链接」与「自定义 HTML / 广告代码」两种内容形式<br>
✅ 支持提示框下方、提示框上方、底部悬浮横幅三种位置，悬浮横幅自带关闭按钮，兼容全部 8 套皮肤<br>
✅ 可仅对未登录访客展示；HTML 广告可选透明容器；设置面板提供「预览跳转页」链接<br>
✅ 外观可自定义：宽度、背景色、文字颜色、强调色、圆角与自定义 CSS；倒计时文案可自定义<br>
✅ 广告 HTML 保存与渲染均经过 `wp_kses` 过滤（允许 `iframe`），第三方广告脚本需显式开启「原样输出」<br>
✅ 图片广告链接强制 http(s) 校验，新窗口打开并附带 `rel="noopener noreferrer nofollow sponsored"`<br>
✅ 新增可中断的倒计时自动跳转（0～30 秒，默认关闭）：点击取消、按任意键或切换标签页均会停止<br>
✅ 提供 `dmy_link_ad_config` / `dmy_link_ad_html` / `dmy_link_ad_allowed_html` / `dmy_link_ad_countdown` 过滤器供主题二次定制<br>
🟢 **性能与稳定性**：<br>
